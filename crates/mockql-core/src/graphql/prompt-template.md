...
2. The response MUST have this shape:
   {"data": <root>, "extensions": {"__MOCK_RESPONSE_EXT__": {"__REASONING_EXT__": <string>}}}
...
3. Unions / Interfaces:
   ...
   - If no fragment-covered concrete types are available, choose a concrete type allowed by the schema.
4. Enums:
...
7. Output constraints:
   ...
   - `extensions.__MOCK_RESPONSE_EXT__.__REASONING_EXT__` must be a single-line string <= 240 characters (no step-by-step).
...
### GraphQL Operation
```graphql
__GRAPHQL_OPERATION__
```
### Variables
__VARIABLES_BLOCK__
### Fields To Mock
__MOCKED_FIELDS_BLOCK__
__OPTIONAL_CONTEXT_SECTIONS__
